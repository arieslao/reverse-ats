...
def _digest_env() -> dict:
    """Load CF_BASE_URL / CF_INGEST_SECRET / recipient — env first, then .digest.env
    (so the email relay keeps working across keepalive restarts)."""
    out = {k: os.environ.get(k, "") for k in ("CF_BASE_URL", "CF_INGEST_SECRET", "CANDIDATE_CONTACT", "EMAIL_TO")}
    f = Path(__file__).parent.parent / ".digest.env"
    if f.exists():
...
def _resume_docx_bytes(job, settings, profile):
    master = (profile.get("resume_text") or "").strip()
    import resume_tailor as rt
    if master and rt.looks_like_master(master):
        from scorer import tailor_master_resume
        ...
        return master_resume_to_docx(
            res.get("name") or os.environ.get("CANDIDATE_NAME", ""),
            res.get("contact", ""), res.get("target_title", ""), res.get("sections", [])), None
    return None, "No master résumé on file — upload it in Admin → Profile first."


@app.post("/api/jobs/{job_id}/email-docs")
def email_docs_endpoint(job_id: str):
    """Generate the tailored résumé + cover letter and email both as .docx attachments
    ...
    try:
        ...
        settings = get_llm_settings(conn)
        profile = get_profile(conn)
        from docgen import slug, cover_to_docx
        attachments = []

        rbytes, rerr = _resume_docx_bytes(job, settings, profile)
        if rbytes:
            attachments.append({"filename": f"resume_{slug(job.get('company',''))}_{slug(job.get('title',''))}.docx",
        ...
        if not cl.get("error") and cl.get("cover_letter"):
            cbytes = cover_to_docx(cl["cover_letter"], os.environ.get("CANDIDATE_NAME", ""),
                                   cfg.get("CANDIDATE_CONTACT", ""), job.get("company", ""))
            attachments.append({"filename": f"cover_{slug(job.get('company',''))}_{slug(job.get('title',''))}.docx",
                                "content": base64.b64encode(cbytes).decode()})

        if not attachments:
            raise HTTPException(status_code=502, detail=rerr or "Could not generate documents.")

        subject = f"{job.get('title','')} @ {job.get('company','')} — tailored résumé + cover letter"
        html = (f"<p>Tailored documents for <b>{job.get('title','')}</b> at <b>{job.get('company','')}</b>"
